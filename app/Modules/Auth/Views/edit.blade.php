...
@include('dashboard.Layouts.breadcrumb',[
    'breadcrumbs' => [
        ...
        [
            'title' => ($blog->{'title_'.LANGUAGE_PREF} != null ? $blog->{'title_'.LANGUAGE_PREF} : $blog->{'title_'.LANGUAGE_PREF2}),
            'url' => \URL::current()
        ],
    ]
])
    ...
    <form class="form" method="post" action="{{ URL::to($designElems['mainData']['url'].'/update/'.$blog->id) }}" enctype="multipart/form-data">
        ...
        <div class="card-body">
            ...
            <div class="form-group mb-3">
                ...
                <input type="text" class="title_ar form-control" value="{{$blog->title_ar}}" name="title_ar" placeholder="{{ trans('main.titleAr') }}">
            </div>
            <div class="form-group mb-3">
                <label for="inputEmail4" class="titleLabel">{{ trans('main.titleEn') }} :</label>
                <input type="text" class="title_en form-control" value="{{$blog->title_en}}" name="title_en" placeholder="{{ trans('main.titleEn') }}">
            </div>
