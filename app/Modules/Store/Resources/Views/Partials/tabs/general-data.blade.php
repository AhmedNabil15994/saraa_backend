...
<div class="tab-content mt-5" id="myTabContent">
    ...
    <div class="tab-pane fade {{$key == 0 ? 'active show' : '' }}" id="home-{{$key+1}}" role="tabpanel" aria-labelledby="home-tab-{{$key+1}}">
        ...
        <div class="form-group p-0 m-0 mb-5 pt-5">
            <label class="col-form-label">{{ trans('Store::store.form.description_'.$lang['prefix']) }} :</label>
            <textarea  class="form-control col-md-8" name="description_{{$lang['prefix']}}" placeholder="{{ trans('Store::store.form.description_'.$lang['prefix']) }}" cols="30" rows="10">{{ isset($model) ? $model->{'description_'.$lang['prefix']} : old('description_'.$lang['prefix']) }}</textarea>
        </div>
    </div>
    @endforeach
</div>
...
<div class="form-group  p-0 m-0 mb-5">
    <label>{{ trans('Store::store.form.seller') }}</label>
    <select name="seller_id" class="form-control col-md-8" data-toggle="select2">
        <option value="">{{trans('Dashboard::dashboard.choose')}}</option>
        @foreach($sellers as $seller)
        <option value="{{$seller->id}}"  {{ $seller->id == (isset($model) ? $model->seller_id : old('seller_id')) ? 'selected' : '' }}>{{$seller->name}}</option>
        @endforeach
    </select>
</div>

<div class="form-group  p-0 m-0 mb-5">
    <label>{{ trans('Store::store.form.employees') }}</label>
    <select name="emp_ids[]" class="form-control col-md-8" multiple data-toggle="select2">
        <option value="">{{trans('Dashboard::dashboard.choose')}}</option>
        @php 
            $storeEmps = isset($model) ? $model->Employees()->pluck('emp_id') : []; 
            $storeEmps = isset($model) ? reset($storeEmps) : [];
        @endphp
        @foreach($employees as $employee)
        <option value="{{$employee->id}}"  {{ in_array($employee->id, $storeEmps) ? 'selected' : '' }}>{{$employee->name}}</option>
        @endforeach
    </select>
</div>
